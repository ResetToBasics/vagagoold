/**
 * @fileoverview Componente de ícones SVG reutilizáveis
 * @description Centraliza todos os ícones utilizados na aplicação
 */

import React from 'react';

/**
 * Props para os componentes de ícone
 */
interface IconeProps {
    /** Largura do ícone em pixels */
    largura?: number;
    /** Altura do ícone em pixels */
    altura?: number;
    /** Classe CSS adicional */
    className?: string;
}

/**
 * Ícone de calendário para representar agendamentos
 */
export function IconeCalendario({ largura = 17, altura = 17, className }: IconeProps) {
    const clipId = React.useId();
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 17 17"
            width={largura}
            height={altura}
            className={className}
            aria-hidden="true"
        >
            <g clipPath={`url(#${clipId})`}>
                <path
                    d="M13.4583 2.01175H12.75V1.36134C12.75 1.18885 12.6754 1.02341 12.5425 0.901437C12.4097 0.779462 12.2295 0.710938 12.0417 0.710938C11.8538 0.710938 11.6736 0.779462 11.5408 0.901437C11.408 1.02341 11.3333 1.18885 11.3333 1.36134V2.01175H5.66667V1.36134C5.66667 1.18885 5.59204 1.02341 5.4592 0.901437C5.32636 0.779462 5.14619 0.710938 4.95833 0.710938C4.77047 0.710938 4.5903 0.779462 4.45747 0.901437C4.32463 1.02341 4.25 1.18885 4.25 1.36134V2.01175H3.54167C2.6027 2.01278 1.70252 2.35574 1.03857 2.96539C0.374625 3.57504 0.00112473 4.40161 0 5.26378L0 13.0687C0.00112473 13.9308 0.374625 14.7574 1.03857 15.3671C1.70252 15.9767 2.6027 16.3197 3.54167 16.3207H13.4583C14.3973 16.3197 15.2975 15.9767 15.9614 15.3671C16.6254 14.7574 16.9989 13.9308 17 13.0687V5.26378C16.9989 4.40161 16.6254 3.57504 15.9614 2.96539C15.2975 2.35574 14.3973 2.01278 13.4583 2.01175ZM1.41667 5.26378C1.41667 4.74629 1.64055 4.24999 2.03906 3.88406C2.43758 3.51814 2.97808 3.31256 3.54167 3.31256H13.4583C14.0219 3.31256 14.5624 3.51814 14.9609 3.88406C15.3595 4.24999 15.5833 4.74629 15.5833 5.26378V5.91419H1.41667V5.26378ZM13.4583 15.0199H3.54167C2.97808 15.0199 2.43758 14.8143 2.03906 14.4484C1.64055 14.0825 1.41667 13.5862 1.41667 13.0687V7.215H15.5833V13.0687C15.5833 13.5862 15.3595 14.0825 14.9609 14.4484C14.5624 14.8143 14.0219 15.0199 13.4583 15.0199Z"
                    fill="black"
                />
                <path
                    d="M8.5 11.4429C9.0868 11.4429 9.5625 11.0061 9.5625 10.4673C9.5625 9.92849 9.0868 9.4917 8.5 9.4917C7.9132 9.4917 7.4375 9.92849 7.4375 10.4673C7.4375 11.0061 7.9132 11.4429 8.5 11.4429Z"
                    fill="black"
                />
                <path
                    d="M4.96094 11.4429C5.54774 11.4429 6.02344 11.0061 6.02344 10.4673C6.02344 9.92849 5.54774 9.4917 4.96094 9.4917C4.37413 9.4917 3.89844 9.92849 3.89844 10.4673C3.89844 11.0061 4.37413 11.4429 4.96094 11.4429Z"
                    fill="black"
                />
                <path
                    d="M12.0391 11.4429C12.6259 11.4429 13.1016 11.0061 13.1016 10.4673C13.1016 9.92849 12.6259 9.4917 12.0391 9.4917C11.4523 9.4917 10.9766 9.92849 10.9766 10.4673C10.9766 11.0061 11.4523 11.4429 12.0391 11.4429Z"
                    fill="black"
                />
            </g>
            <defs>
                <clipPath id={clipId}>
                    <rect width="17" height="17" rx="5" fill="white" />
                </clipPath>
            </defs>
        </svg>
    );
}

/**
 * Ícone de usuários/grupo para representar clientes
 */
export function IconeUsuarios({ largura = 20, altura = 20, className }: IconeProps) {
    const clipId = React.useId();

    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 20 20"
            width={largura}
            height={altura}
            className={className}
            aria-hidden="true"
        >
            <g clipPath={`url(#${clipId})`}>
                <path
                    d="M10 13.3333C9.34073 13.3333 8.69626 13.1378 8.1481 12.7716C7.59994 12.4053 7.17269 11.8847 6.9204 11.2756C6.66811 10.6665 6.6021 9.9963 6.73072 9.3497C6.85933 8.7031 7.1768 8.10915 7.64298 7.64298C8.10915 7.1768 8.7031 6.85933 9.3497 6.73072C9.9963 6.6021 10.6665 6.66811 11.2756 6.9204C11.8847 7.1727 12.4053 7.59994 12.7716 8.1481C13.1378 8.69627 13.3333 9.34073 13.3333 10C13.3333 10.8841 12.9821 11.7319 12.357 12.357C11.7319 12.9821 10.8841 13.3333 10 13.3333ZM10 8.33333C9.67036 8.33333 9.34813 8.43108 9.07405 8.61422C8.79997 8.79735 8.58635 9.05765 8.4602 9.3622C8.33405 9.66674 8.30105 10.0019 8.36536 10.3252C8.42967 10.6485 8.5884 10.9454 8.82149 11.1785C9.05458 11.4116 9.35155 11.5703 9.67485 11.6346C9.99815 11.699 10.3333 11.6659 10.6378 11.5398C10.9423 11.4137 11.2026 11.2 11.3858 10.926C11.5689 10.6519 11.6667 10.3296 11.6667 10C11.6667 9.55797 11.4911 9.13405 11.1785 8.82149C10.8659 8.50893 10.442 8.33333 10 8.33333ZM15 19.1667C15 17.8406 14.4732 16.5688 13.5355 15.6311C12.5979 14.6935 11.3261 14.1667 10 14.1667C8.67392 14.1667 7.40215 14.6935 6.46447 15.6311C5.52678 16.5688 5 17.8406 5 19.1667C5 19.3877 5.0878 19.5996 5.24408 19.7559C5.40036 19.9122 5.61232 20 5.83333 20C6.05435 20 6.26631 19.9122 6.42259 19.7559C6.57887 19.5996 6.66667 19.3877 6.66667 19.1667C6.66667 18.2826 7.01786 17.4348 7.64298 16.8096C8.2681 16.1845 9.11594 15.8333 10 15.8333C10.8841 15.8333 11.7319 16.1845 12.357 16.8096C12.9821 17.4348 13.3333 18.2826 13.3333 19.1667C13.3333 19.3877 13.4211 19.5996 13.5774 19.7559C13.7337 19.9122 13.9457 20 14.1667 20C14.3877 20 14.5996 19.9122 14.7559 19.7559C14.9122 19.5996 15 19.3877 15 19.1667ZM15 6.66667C14.3407 6.66667 13.6963 6.47117 13.1481 6.1049C12.5999 5.73863 12.1727 5.21803 11.9204 4.60895C11.6681 3.99986 11.6021 3.32964 11.7307 2.68303C11.8593 2.03643 12.1768 1.44249 12.643 0.976312C13.1092 0.510137 13.7031 0.192668 14.3497 0.0640506C14.9963 -0.0645668 15.6665 0.00144427 16.2756 0.253736C16.8847 0.506029 17.4053 0.93327 17.7716 1.48143C18.1378 2.0296 18.3333 2.67406 18.3333 3.33334C18.3333 4.21739 17.9821 5.06524 17.357 5.69036C16.7319 6.31548 15.8841 6.66667 15 6.66667ZM15 1.66667C14.6704 1.66667 14.3481 1.76442 14.074 1.94755C13.8 2.13069 13.5863 2.39099 13.4602 2.69553C13.3341 3.00007 13.301 3.33518 13.3654 3.65849C13.4297 3.98179 13.5884 4.27876 13.8215 4.51185C14.0546 4.74493 14.3515 4.90367 14.6748 4.96798C14.9982 5.03229 15.3333 4.99928 15.6378 4.87313C15.9423 4.74699 16.2026 4.53337 16.3858 4.25929C16.5689 3.9852 16.6667 3.66297 16.6667 3.33334C16.6667 2.89131 16.4911 2.46738 16.1785 2.15482C15.8659 1.84226 15.442 1.66667 15 1.66667ZM20 12.5C19.9987 11.1743 19.4715 9.90333 18.5341 8.96593C17.5967 8.02853 16.3257 7.50133 15 7.5C14.779 7.5 14.567 7.5878 14.4107 7.74408C14.2545 7.90036 14.1667 8.11232 14.1667 8.33333C14.1667 8.55435 14.2545 8.76631 14.4107 8.92259C14.567 9.07887 14.779 9.16667 15 9.16667C15.8841 9.16667 16.7319 9.51786 17.357 10.143C17.9821 10.7681 18.3333 11.6159 18.3333 12.5C18.3333 12.721 18.4211 12.933 18.5774 13.0893C18.7337 13.2455 18.9457 13.3333 19.1667 13.3333C19.3877 13.3333 19.5996 13.2455 19.7559 13.0893C19.9122 12.933 20 12.721 20 12.5ZM5 6.66667C4.34073 6.66667 3.69626 6.47117 3.1481 6.1049C2.59994 5.73863 2.17269 5.21803 1.9204 4.60895C1.66811 3.99986 1.6021 3.32964 1.73072 2.68303C1.85933 2.03643 2.1768 1.44249 2.64298 0.976312C3.10915 0.510137 3.7031 0.192668 4.3497 0.0640506C4.9963 -0.0645668 5.66652 0.00144427 6.27561 0.253736C6.8847 0.506029 7.40529 0.93327 7.77156 1.48143C8.13784 2.0296 8.33333 2.67406 8.33333 3.33334C8.33333 4.21739 7.98214 5.06524 7.35702 5.69036C6.7319 6.31548 5.88405 6.66667 5 6.66667ZM5 1.66667C4.67036 1.66667 4.34813 1.76442 4.07405 1.94755C3.79997 2.13069 3.58635 2.39099 3.4602 2.69553C3.33405 3.00007 3.30105 3.33518 3.36536 3.65849C3.42967 3.98179 3.5884 4.27876 3.82149 4.51185C4.05458 4.74493 4.35155 4.90367 4.67485 4.96798C4.99815 5.03229 5.33326 4.99928 5.63781 4.87313C5.94235 4.74699 6.20265 4.53337 6.38578 4.25929C6.56892 3.9852 6.66667 3.66297 6.66667 3.33334C6.66667 2.89131 6.49107 2.46738 6.17851 2.15482C5.86595 1.84226 5.44203 1.66667 5 1.66667ZM1.66667 12.5C1.66667 11.6159 2.01786 10.7681 2.64298 10.143C3.2681 9.51786 4.11594 9.16667 5 9.16667C5.22101 9.16667 5.43297 9.07887 5.58926 8.92259C5.74554 8.76631 5.83333 8.55435 5.83333 8.33333C5.83333 8.11232 5.74554 7.90036 5.58926 7.74408C5.43297 7.5878 5.22101 7.5 5 7.5C3.67432 7.50133 2.40332 8.02853 1.46593 8.96593C0.528533 9.90333 0.00132369 11.1743 0 12.5C0 12.721 0.0877974 12.933 0.244078 13.0893C0.400358 13.2455 0.61232 13.3333 0.833333 13.3333C1.05435 13.3333 1.26631 13.2455 1.42259 13.0893C1.57887 12.933 1.66667 12.721 1.66667 12.5Z"
                    fill="black"
                />
            </g>
            <defs>
                <clipPath id={clipId}>
                    <rect width="20" height="20" fill="white" />
                </clipPath>
            </defs>
        </svg>
    );
}

/**
 * Ícone de usuário individual
 */
export function IconeUsuario({ largura = 20, altura = 20, className }: IconeProps) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            width={largura}
            height={altura}
            stroke="currentColor"
            strokeWidth="2"
            className={className}
            aria-hidden="true"
        >
            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
            <circle cx="12" cy="7" r="4" />
        </svg>
    );
}

/**
 * Ícone de lista com checkmarks para logs
 */
export function IconeLista({ largura = 20, altura = 20, className }: IconeProps) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            width={largura}
            height={altura}
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            className={className}
            aria-hidden="true"
        >
            <line x1="10" y1="6" x2="21" y2="6" />
            <line x1="10" y1="12" x2="21" y2="12" />
            <line x1="10" y1="18" x2="21" y2="18" />
            <polyline points="3 6 4 7 6 5" />
            <polyline points="3 12 4 13 6 11" />
            <polyline points="3 18 4 19 6 17" />
        </svg>
    );
}

/**
 * Ícone de busca/lupa
 */
export function IconeBusca({ largura = 17, altura = 17, className }: IconeProps) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 17 17"
            width={largura}
            height={altura}
            className={className}
            aria-hidden="true"
        >
            <path
                d="M7.5 0C11.64 0 15 3.36 15 7.5C15 11.64 11.64 15 7.5 15C3.36 15 0 11.64 0 7.5C0 3.36 3.36 0 7.5 0ZM7.5 13.3333C10.7225 13.3333 13.3333 10.7225 13.3333 7.5C13.3333 4.27667 10.7225 1.66667 7.5 1.66667C4.27667 1.66667 1.66667 4.27667 1.66667 7.5C1.66667 10.7225 4.27667 13.3333 7.5 13.3333ZM14.5708 13.3925L16.9283 15.7492L15.7492 16.9283L13.3925 14.5708L14.5708 13.3925Z"
                fill="black"
            />
        </svg>
    );
}

/**
 * Ícone de relógio
 */
export function IconeRelogio({ largura = 18, altura = 18, className }: IconeProps) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            width={largura}
            height={altura}
            stroke="currentColor"
            strokeWidth="2"
            className={className}
            aria-hidden="true"
        >
            <circle cx="12" cy="12" r="10" />
            <polyline points="12 6 12 12 16 14" />
        </svg>
    );
}

/**
 * Ícone de seta para baixo (chevron)
 */
export function IconeSetaBaixo({ largura = 20, altura = 20, className }: IconeProps) {
    return (
        <svg
            width={largura}
            height={altura}
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            className={className}
            aria-hidden="true"
        >
            <path d="M6 9l6 6 6-6" />
        </svg>
    );
}

/**
 * Ícone de fechar (X)
 */
export function IconeFechar({ largura = 18, altura = 18, className }: IconeProps) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            width={largura}
            height={altura}
            stroke="currentColor"
            strokeWidth="2"
            className={className}
            aria-hidden="true"
        >
            <line x1="18" y1="6" x2="6" y2="18" />
            <line x1="6" y1="6" x2="18" y2="18" />
        </svg>
    );
}

/**
 * Ícone de olho para alternar visibilidade de senha
 */
export function IconeOlhoSenha({ largura = 24, altura = 21, className }: IconeProps) {
    return (
        <svg
            width={largura}
            height={altura}
            viewBox="0 0 24 21"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={className}
            aria-hidden="true"
        >
            <path
                d="M13.9407 8.1876L9.08561 12.5624C8.46193 12.0004 8.07812 11.2309 8.07812 10.375C8.07812 8.66312 9.61333 7.27979 11.5132 7.27979C12.4631 7.27979 13.317 7.62562 13.9407 8.1876Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M17.0981 4.98885C15.4189 3.8476 13.4999 3.2251 11.5138 3.2251C8.1267 3.2251 4.96993 5.02343 2.77267 8.13593C1.90911 9.35499 1.90911 11.4041 2.77267 12.6231C3.53067 13.6952 4.41342 14.6203 5.37292 15.3638"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M8.07812 16.8851C9.17196 17.3001 10.333 17.5249 11.5132 17.5249C14.9002 17.5249 18.057 15.7266 20.2542 12.6141C21.1178 11.395 21.1178 9.34596 20.2542 8.12689C19.9376 7.67731 19.5922 7.25367 19.2372 6.85596"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M14.8816 10.98C14.6321 12.199 13.5287 13.1933 12.1758 13.4181"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M9.08547 12.5625L1.91797 19.0209"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M21.1089 1.729L13.9414 8.18744"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
        </svg>
    );
}

/**
 * Logo da aplicação VagaGoold
 */
export function LogoVagaGoold({ largura = 52, altura = 52, className }: IconeProps) {
    return (
        <svg
            width={largura}
            height={altura}
            viewBox="0 0 52 52"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={className}
            role="img"
            aria-label="VagaGoold"
        >
            <path
                d="M17.3794 20.5035C24.1544 17.2661 29.4565 11.7724 32.5004 4.90515C30.5366 2.84499 27.9837 1.37344 25.2344 0.490515L23.7616 0C23.6634 0.196206 23.5652 0.490527 23.5652 0.686733C21.2087 7.35774 16.3975 12.7534 9.91704 15.7946C5.40037 17.9529 2.06196 21.877 0.490942 26.5859L0 28.0575C0.196377 28.1556 0.490947 28.2537 0.687324 28.2537C2.5529 28.9404 4.41848 29.8233 6.18588 30.9025C8.64059 26.4878 12.5681 22.7599 17.3794 20.5035Z"
                fill="#201B21"
            />
            <path
                d="M45.8124 20.8955C42.1794 26.3893 37.1718 30.8039 30.9859 33.747C25.9783 36.1015 22.149 40.5162 20.3816 45.8137L19.8906 47.4815C21.7562 49.1492 24.0145 50.4246 26.4692 51.2094L27.9421 51.6999C28.0403 51.5037 28.1384 51.2094 28.1384 51.0132C30.495 44.3422 35.3062 38.9465 41.7867 35.9053C46.3033 33.747 49.7399 29.8229 51.2127 25.114L51.7037 23.6424C49.6417 23.0538 47.678 22.0727 45.8124 20.8955Z"
                fill="#201B21"
            />
            <path
                d="M15.9986 42.3794C18.4533 36.4933 22.9699 31.6862 28.7631 28.9393C34.0652 26.3886 38.4837 22.4645 41.5276 17.6575C39.2692 15.4992 37.4036 13.0466 36.029 10.1035C32.396 16.7745 26.7011 22.1702 19.7297 25.5057C15.704 27.3697 12.4638 30.6071 10.5 34.5312C12.6601 36.5914 14.4275 39.142 15.8022 41.987C15.8022 41.987 15.9004 42.1832 15.9986 42.3794Z"
                fill="#201B21"
            />
        </svg>
    );
}
